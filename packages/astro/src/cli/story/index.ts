...
export async function story({ flags, logging }: DevOptions) {
	const settings = await loadSettings({ cmd: 'story', flags, logging });
	if (!settings) return;
	...
	return await devServer(settings, {
		...
		logging,
		handleConfigError(e) {
			handleConfigError(e, { cmd: 'story', cwd: root, flags, logging });
